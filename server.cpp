...
{
    std::cout<<"init_stateful_proxy "<<std::endl;
    pj_status_t status;
...
static pj_bool_t proxy_on_rx_request( pjsip_rx_data *rdata )
{
    std::cout<<"proxy_on_rx_request "<<std::endl;
    ...
    if (rdata->msg_info.msg->line.req.method.id != PJSIP_CANCEL_METHOD)
    {
        ...
        if (status != PJ_SUCCESS)
        {
            std::cout<<" pjsip_endpt_create_request_fwd_error" <<std::endl;
        ...
        if (status != PJ_SUCCESS)
        {
            std::cout<<" proxy_process_routing_error" <<std::endl;
        ...
        ///
        ///
        status = proxy_calculate_subscriptions(rdata,tdata);
        if(status != PJ_SUCCESS)
        {
            printf("status code is : %d\n",*((int *)&status));
            app_perror("Error calculating target", status);
         ...
         */
        uac_data = (struct uac_data*) pj_pool_alloc(uac_tsx->pool, sizeof(struct uac_data));
         ...
         */
        uas_data = (struct uas_data*)pj_pool_alloc(uas_tsx->pool, sizeof(struct uas_data));
        ...
        * */
        ...
        if (rdata->msg_info.msg->line.req.method.id == PJSIP_INVITE_METHOD)
        {
            ...
            pjsip_endpt_create_response(global.endpt, rdata, 100, NULL, &res100);
    ...
    else
    {
        ...
        pjsip_tsx_create_key(rdata->tp_info.pool, &key, PJSIP_UAS_ROLE, pjsip_get_invite_method(), rdata);
        ...
        pjsip_endpt_respond(global.endpt, NULL, rdata, 200, NULL, NULL, NULL, NULL);
        ...
        if (uas_data2->uac_tsx && uas_data2->uac_tsx->status_code < 200)
        {
            ...
            pjsip_endpt_create_cancel(global.endpt, uas_data2->uac_tsx->last_tx, &cancel);
...
static pj_bool_t proxy_on_rx_response( pjsip_rx_data *rdata )
{
    std::cout<<"proxy_on_rx_response"<<std::endl;
    pjsip_tx_data *tdata;
...
static void tu_on_tsx_state(pjsip_transaction *tsx, pjsip_event *event)
{
    std::cout<<"on transaction state changed "<<std::endl;
    struct uac_data *uac_data;
    pj_status_t status;
    ...
    if (event->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
         ...
         * INVITE with 100)
         */
        if (tsx->method.id == PJSIP_INVITE_METHOD && rdata->msg_info.msg->line.status.code == 100)
        {
            return;
        }
     ...
     */
    if (tsx->state == PJSIP_TSX_STATE_TERMINATED && uac_data && uac_data->uas_tsx)
    {
